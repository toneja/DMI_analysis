...
def batch_process(image_folder):
    ...
    for folder_name in os.listdir(image_folder):
        ...
        if os.path.isdir(current_folder):
            ...
            if os.path.exists(f"DMI/images/{folder_name}") and os.path.exists(
                f"DMI/results/{folder_name}"
            ):
                print(f"Skipping folder: {current_folder}, already processed.")
                continue

            # make output folders
            os.makedirs(f"DMI/images/{folder_name}", exist_ok=True)
            os.makedirs(f"DMI/results/{folder_name}", exist_ok=True)
            ...
            for file in os.listdir("DMI/images"):
                if file.endswith(".tif"):
                    os.replace(f"DMI/images/{file}", f"DMI/images/{folder_name}/{file}")
            for file in os.listdir("DMI/results"):
                if file.endswith(".csv"):
                    os.replace(
                        f"DMI/results/{file}", f"DMI/results/{folder_name}/{file}"
                    )
